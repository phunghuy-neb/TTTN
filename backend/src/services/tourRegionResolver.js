...
// Keep punctuation as a hard boundary. TPHCM is the only supported dotted abbreviation.
...
export function isForeignTourName(value) {
...
function isExcludedSegment(segment) {
  ...
  // These mentions describe transport or a road, not a visited destination.
  if (hasPhrase(text, [
    ...
    'theo duong ho chi minh',
    'huong dong bac', 'phia dong bac', 'huong tay bac', 'phia tay bac',
    'huong bac', 'phia bac', 'chu tich ho chi minh', 'ho chi minh tuyen ngon',
    'bao tang ho chi minh', 'lang chu tich ho chi minh', 'nha san bac ho',
  ])) return true

  // "Phật Thích ca Mâu ni" is not the southern province Cà Mau.
...
function regionsFromItineraryDay(day = {}) {
  ...
  const returnContext = hasReturnContext(context)
  const departureContext = hasDepartureOriginContext(context)

  if (titleSegments.length > 1 && departureContext) {
    titleSegments = titleSegments.slice(1)
  }
  if (titleSegments.length > 0 && returnContext) {
    titleSegments = titleSegments.slice(0, -1)
  }
  if (titleSegments.length === 1 && returnContext && isExcludedSegment(description)) {
    titleSegments = []
  }
...
function regionsFromTourName(value) {
  ...
  const routePart = name.slice(colonIndex + 1)
  const routeRegions = regionsFromRouteSegments(routePart, {
...
/**
 * Resolve all Vietnamese regions represented by actual tour destinations.
 * The returned array is runtime-only and is never persisted to MongoDB.
 */
export function resolveTourRegions(tour = {}) {
